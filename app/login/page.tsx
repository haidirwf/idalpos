...
function LoginFormContent() {
  ...
  return (
    <div className="w-full max-w-md bg-[#18181B] border border-neutral-800 rounded-2xl p-8 shadow-2xl relative z-10">
      ...
      {registered && !error && (
        <div className="mb-6 flex items-start gap-3 bg-emerald-500/10 border border-emerald-500/20 text-emerald-200 p-4 rounded-xl text-sm animate-in fade-in duration-200">
          <CheckCircle className="w-5 h-5 shrink-0 text-emerald-400" />
          <div>
            <p className="font-semibold text-emerald-400">Registrasi Berhasil!</p>
            <p className="mt-0.5 text-neutral-350 leading-relaxed">
              Akun admin Anda berhasil dibuat. Silakan konfirmasi email (jika diaktifkan) atau langsung masuk di bawah ini.
            </p>
          </div>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div className="space-y-2">
          ...
          <div className="relative">
            ...
            <input
              ...
              required
              disabled={isPending}
              className="w-full bg-[#0F0F10] border border-neutral-800 rounded-xl pl-11 pr-4 py-3 text-white placeholder-neutral-500 focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 transition-all disabled:opacity-50 disabled:cursor-not-allowed text-sm"
        ...
        <button
          ...
          type="submit"
          disabled={isPending}
          className="w-full bg-amber-500 hover:bg-amber-600 text-black font-bold py-3.5 px-6 rounded-xl transition-all shadow-lg hover:shadow-amber-500/10 active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 cursor-pointer mt-2 text-sm"
        >
          {isPending ? (
            <>
      ...
      <div className="text-center mt-6 text-xs text-neutral-500">
        ...
        <Link href="/register" className="text-amber-500 hover:text-amber-400 font-semibold transition-all">
          Daftar Sekarang
        </Link>
      </div>
    </div>
...
export default function LoginPage() {
  return (
    <div className="min-h-screen bg-[#0F0F10] flex items-center justify-center p-4 text-white relative overflow-hidden font-sans">
      ...
      <Suspense fallback={<div className="text-neutral-500 text-sm">Loading login form...</div>}>
        <LoginFormContent />
      </Suspense>
    </div>
  );
